...
class IntroductionViewModel @Inject constructor(
    ...
    override fun handleEvent(event: IntroductionContract.Event) {
        when (event) {
            ...
            is IntroductionContract.Event.ApplyPromoCode -> applyPromoCode(event.promoCode)
    ...
    private fun applyPromoCode(promoCode: String?) {
        viewModelScope.launch {
            // Здесь могла бы быть логика применения промокода
            setEffect { IntroductionContract.Effect.PromoCodeApplied(promoCode ?: "none") }
        }
    }
